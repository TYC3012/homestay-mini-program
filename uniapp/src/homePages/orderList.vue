            ...
            <view v-for="(item,index) in orderList" :key="index" class="order__item">
              ...
              <view class="order__item__operation tn-flex tn-flex-direction-row tn-flex-nowrap tn-flex-col-center tn-flex-row-between">
                <view class="order__item__operaation__left">
                  <text class="order__item__operaation__left--text" style="color: red;" @click="onClick(item)">删除</text>
                </view>
                <view class="order__item__operaation__left">
                ...
                <view class="order__item__operation__right tn-flex tn-flex-direction-row tn-flex-nowrap tn-flex-col-center tn-flex-row-right">
                  <view class="order__item__operation__right__button">
                    <tn-button v-if="item.status ===2 " shadow shape="round" fontColor="tn-color-white" backgroundColor="tn-bg-cyan"
                               :fontSize="24" height="auto" padding="10rpx 18rpx"
                               @click="tn('/homePages/evaluate',item.id)">评价</tn-button>
                  </view>
                </view>
              </view>
            </view>
...
import {userOrderDelete} from "../api/modules/user";
...
export default {
  ...
  methods: {
    ...
    clickBtn(event) {
      ...
      console.log(event)
      if(event.index ===1 ){
        console.log('确认删除',this.clickOrder);
        //return
        let that=this;
        that.$api.userOrderDelete(this.clickOrder).then(res =>{
